...
public class Manager : MonoBehaviour
{
    ...
    {      
		...
		monsterBlockObjects = new GameObject[3,1];
	...
	private void displayYellowLine(BlockPosition pos,float rot,int type){	//type 1 horical, 2 vertical 3 cross l-r 4 cross r-l
		...
		Quaternion qua= Quaternion.Euler(rot,90.0f,0.5f);
		if(type==1){
			pos3d=new Vector3(pos.x-2.5f, pos.y-4f, 0.5f);
			qua= Quaternion.Euler(rot,90.0f,0.5f);
		}
		
		if(type==2){
			pos3d=new Vector3(pos.x-3, pos.y-3.5f, 0.5f);
			qua= Quaternion.Euler(rot,90.0f,0.5f);
		}
		
		if(type==3){
			pos3d=new Vector3(2, 1, 0.5f);
			qua= Quaternion.Euler(rot,270.0f,0.5f);
		}
		
		if(type==4){
			pos3d=new Vector3(-3.0f,1.0f, 0.5f);
			qua= Quaternion.Euler(rot,90.0f,0.5f);
		}
